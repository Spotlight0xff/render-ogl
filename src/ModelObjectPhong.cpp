...
ModelObjectPhong::ModelObjectPhong(Model* m, SceneObject const& l, Scene* s)
: ModelObject(m, s),
  light(l) {
  setShader("phong_model",
         [this](Scene& scene, ModelObject& obj, Shader& shader) {
           ...
           glUniform3fv(glGetUniformLocation(shader.getId(), "lightColor"), 1, glm::value_ptr(this->light.getColor()));
